...
sealed interface BoundTypeArgument {
    val astNode: AstTypeArgument
    val variance: TypeVariance get() = astNode.variance

    class Named(
        val context: CTContext,
        override val astNode: AstTypeArgument.Reference,
        val type: BoundTypeReference,
    ) : BoundTypeReference, BoundTypeArgument {
        init {
        ...
        override fun unify(
            ...
            if (assigneeType !is BoundTypeArgument && this.variance == TypeVariance.OUT) {
                return carry.plusReporting(
                    ValueNotAssignableDiagnostic(
                        this,
                        assigneeType,
                        "Cannot assign to a reference of an out-variant type",
                        assignmentLocation
                    )
                )
            }
            ...
            when (assigneeType) {
                ...
                is BoundTypeArgument -> {
                    ...
                    if (this.variance == TypeVariance.UNSPECIFIED) {
                        ...
                        val assigneeActualTypeNotNullable =
                            assigneeType.type.withCombinedNullability(TypeReference.Nullability.NOT_NULLABLE)

                        // target needs to use the type in both IN and OUT fashion -> source must match exactly
                        if (assigneeActualTypeNotNullable !is GenericTypeReference && !assigneeActualTypeNotNullable.hasSameBaseTypeAs(
                                this.type
                            )
                        ) {
                            return carry2.plusReporting(
                                ValueNotAssignableDiagnostic(
                                    this,
                                    assigneeType,
                                    "the exact type ${this.type} is required",
                                    assignmentLocation
                                )
                            )
                        }
                        ...
                        if (assigneeType.variance != TypeVariance.UNSPECIFIED) {
                            return carry2.plusReporting(
                                ValueNotAssignableDiagnostic(
                                    this,
                                    assigneeType,
                                    "cannot assign an in-variant value to an exact-variant reference",
                                    assignmentLocation
                                )
                            )
                        }
                    ...
                    if (this.variance == TypeVariance.OUT) {
                        ...
                        return carry.plusReporting(
                            ValueNotAssignableDiagnostic(
                                this,
                                assigneeType,
                                "cannot assign in-variant value to out-variant reference",
                                assignmentLocation
                            )
                        )
                    }
                    ...
                    return carry.plusReporting(
                        ValueNotAssignableDiagnostic(
                            this,
                            assigneeType,
                            "cannot assign out-variant value to in-variant reference",
                            assignmentLocation
                        )
                    )
                }
        ...
        fun toBackendIrAsTypeArgument(): IrParameterizedType.Argument =
            IrTypeArgumentImpl(variance.backendIr, type.toBackendIr())
    ...
    /**
     * A wildcard type argument, but it is not yet known what [BoundTypeParameter] it relates to; which in turn means
     * the actual type that this wildcard will stand for is not yet known. It becomes known with the first call
     * to [unify], at which point [ResolvedWildcard] comes into play.
     */
    class UnresolvedWildcard(
        val astNode: AstTypeArgument,
    )

    /**
     * A wildcard type argument for which the relating [BoundTypeParameter] is known; hence, the wildcard semantically
     * acts as if the bound type was mentioned explicitly as a [Named].
     */

...
3. extend OO model
   ...
   3. improve error messages on purity violations, see e.g. the test PurityErrors ... "nesting depth 3";
      this one is currently almost indecipherable to anyone who doesn't already know what's wrong
   4. add accessor-based member variables to interfaces
   5. ~~review the vtable approach: does looking for a prefix suffice to keep them small?~~
   ...
   6. deal with the wrapper mutability problem: do types need to be generic on mutability?

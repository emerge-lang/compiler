...
1. assure the type system is sound: fix the aliasing problem with exclusive references
   1. add `ref` and `own` class field semantics
      * `own` is the default
      * `own` must be initialized with an expr that evalutes to exclusive, or with `init`
      * `own` fields cannot have explicit mutability
      * `own` fields get the automatic ctor type variable treatment so the mutability of the parent
        object gets derived from the mutability of the init values
      * `ref` fields always need an explicit mutability, don't assume `read`
      * `ref` fields can always be dereferenced with their declaread mutability, even if the parent is `const`
   2. fix the aliasing problem by narrowing the mutability of field access through `exclusive` references
      1. add implicit decoration semantics. `= init` is decoration, `= Nested()` is non-decorated
      2. narrow down the mutability obtained when accessing `own` fields on `exclusive` references
         * decorated field always derefence as `read` because the actual mutability isn't known (comes from ctor type variable)
         * undecorated fields dereference depending on whether they're borrowed or not:

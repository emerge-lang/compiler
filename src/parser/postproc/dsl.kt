        ...
        override fun tryMatch(input: TokenSequence): MatchingResult<A> {
            ...
            if (baseResult.isSuccess) {
                ...
                if (baseResult.result == null) {
                    ...
                    // we have to drop the result :// TODO: evaluate whether throwing an exception makes more sense
 ...
 * into a single [TokenSequence] and returns that as the matching result.
...
fun Rule<TokenSequence>.trimWhitespace(front: Boolean = true, back: Boolean = true): Rule<TokenSequence> {
    val isWhitespace: (Token) -> Boolean = { OperatorToken(lexer.Operator.NEWLINE) == it }
